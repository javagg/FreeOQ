...
namespace FreeQuant.FinChart
{
	public class DSView : SeriesView
	{
		...
		private Color KNRy1kSrcC = Color.White;
		private SmoothingMode IXfyvDxxVL = SmoothingMode.AntiAlias;
		private EIndexOption option = EIndexOption.Null;
		...
		public override double LastValue
		{
			get
			{
				...
				if (this.option == EIndexOption.Next)
					return this.mainSeries[this.lastDate.AddTicks(1), EIndexOption.Next];
				else
					return -1.0;
			}
		}
...
//			this.ToolTipFormat = this.toolTipFormat.Replace(FJDHryrxb1WIq5jBAt.mT707pbkgT(2828), pad.Chart.LabelDigitsCount.ToString());
		...
		{
			Pen pen = new Pen(this.mainSeries.Color, this.mainSeries.DrawWidth);
			...
			for (int index3 = index1; index3 <= index2; ++index3)
			{
				...
				if (this.Style == SimpleDSStyle.Bar)
				{
					...
					this.pad.Graphics.FillRectangle(new SolidBrush(this.mainSeries.Color), (float)(x1 - num7 / 2), y, (float)num7, Math.Abs(y - num8));
				}
				if (this.Style == SimpleDSStyle.Circle)
				{
					...
					this.pad.Graphics.FillEllipse(new SolidBrush(this.mainSeries.Color), x1 - this.mainSeries.DrawWidth, num2 - this.mainSeries.DrawWidth, this.mainSeries.DrawWidth * 2, this.mainSeries.DrawWidth * 2);
				}
				++num1;
			}
			if (this.selected)
			{
				...
				while (index3 < arrayList.Count)
				{
					...
					if (this.mainSeries.Contains((DateTime)arrayList[index3]))
					{
						...
						this.pad.Graphics.FillRectangle(new SolidBrush(Color.FromArgb((int)midnightBlue.R ^ (int)byte.MaxValue, (int)midnightBlue.G ^ (int)byte.MaxValue, (int)midnightBlue.B ^ (int)byte.MaxValue)), num9 - 2, num10 - 2, 4, 4);
					}
					index3 += num8;
				}
			}
