// Type: SmartQuant.Testing.Pertrac.Skewness
...
namespace SmartQuant.Testing.Pertrac
{
  public class Skewness : SeriesTesterItem
  {
    ...
    public Skewness(string name, SeriesTesterItem parentSeriesItem)
    {
      ...
      base.\u002Ector(name, parentSeriesItem, s3j2vikrJi2pVH1Xpv.aMieSmUS9G(1350) + parentSeriesItem.Series.Title);
    }
        ...
        if (!Calendar.IsWeekend(this.parentSeries.GetDateTime(index1)))
        {
          ...
          for (int index2 = 0; index2 <= index1; ++index2)
          {
            if (!Calendar.IsWeekend(this.parentSeries.GetDateTime(index2)))
            {
              ...
              num2 += Math.Pow(this.parentSeries[index2] - this.mean, 3.0);
            }
          }
          double x = Math.Sqrt(num1 / (this.n - 1.0));
          if (this.n > 2.0)
            this.series.Add(this.parentSeries.GetDateTime(index1), this.n / ((this.n - 1.0) * (this.n - 2.0)) * num2 / Math.Pow(x, 3.0));
        }
      }
    }
    ...
    protected override double GetValue(DateTime date)
    {
      ...
      for (int index2 = 0; index2 <= index1; ++index2)
      {
        if (!Calendar.IsWeekend(this.parentSeries.GetDateTime(index2)))
        {
          ...
          num6 += Math.Pow(this.parentSeries[index2] - num3, 3.0);
          ++num4;
        }
      }
      double x = Math.Sqrt(num5 / (num4 - 1.0));
      if (num4 > 2.0)
        return num4 / ((num4 - 1.0) * (num4 - 2.0)) * num6 / Math.Pow(x, 3.0);
      else
        return double.NaN;
    }
